...
class WireManager {
  ...
  // ── Per-frame update ───────────────────────────────────────────────────────
  update() {
    // Keep wire endpoints in sync with component positions
    ...
    // Update preview wire if dragging
  ...
  _startDrag(fromSphere) {
    ...
    const dummy = [new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3()]
    const { line } = buildWireLine(dummy, PREVIEW_COLOR)
    line.material.color.set(PREVIEW_COLOR)
    ...
    fromSphere.getWorldPosition(wp)
    this._previewMid.copy(wp)
  }
